...
impl TMF663ShoppingCartManagement {
    ...
    pub async fn add_cart(&self, item : ShoppingCart) -> Result<Vec<ShoppingCart>,PlatypusError> {
        let result = self.persist.as_ref().unwrap().create_tmf_item(item.clone()).await;
        // #[cfg(feature = "events")]
        // {
    ...
    pub async fn get_carts(&self, query_opts : QueryOptions) -> Result<Vec<ShoppingCart>,PlatypusError> {
        self.persist.as_ref().unwrap().get_items(query_opts).await
    }

    pub async fn get_cart(&self, id : String, query_opts : QueryOptions) -> Result<Vec<ShoppingCart>,PlatypusError> {
        self.persist.as_ref().unwrap().get_item(id,query_opts).await
    }

    pub async fn update_cart(&self, id : String, patch : ShoppingCart) -> Result<Vec<ShoppingCart>,PlatypusError> {
