...
use crate::{common::{error::PlatypusError, persist::Persistence}, QueryOptions};
...
impl TMF622ProductOrderManagement {
    ...
    pub async fn add_order(&self, item : ProductOrder) -> Result<Vec<ProductOrder>,PlatypusError> {
        self.persist.as_ref().unwrap().create_tmf_item_lastupdate(item).await
    }

    pub async fn get_orders(&self, query_opts : QueryOptions) -> Result<Vec<ProductOrder>,PlatypusError> {
        self.persist.as_ref().unwrap().get_items(query_opts).await
    }

    pub async fn get_order(&self, id : String, query_opts : QueryOptions) -> Result<Vec<ProductOrder>,PlatypusError> {
        self.persist.as_ref().unwrap().get_item(id, query_opts).await
    }

    pub async fn update_order(&self, id : String, patch : ProductOrder) -> Result<Vec<ProductOrder>,PlatypusError> {
