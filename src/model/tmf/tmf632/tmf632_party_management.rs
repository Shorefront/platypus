...
impl TMF632PartyManagement {
    ...
    ) -> Result<Vec<Individual>, PlatypusError> {
        let result = self
            .persist
            .as_ref()
            .unwrap()
            .create_tmf_item(individual.clone())
            .await;
        #[cfg(feature = "events")]
        {
    ...
    ) -> Result<Vec<Organization>, PlatypusError> {
        let result = self
            .persist
            .as_ref()
            .unwrap()
            .create_tmf_item(organization.clone())
            .await;
        #[cfg(feature = "events")]
        {
    ...
    pub async fn delete_organization(&self, id: String) -> Result<Organization, PlatypusError> {
        let result = self
            ...
            .as_ref()
            .unwrap()
            .delete_tmf_item::<Organization>(id)
            .await;
        #[cfg(feature = "events")]
        {
            if let Ok(d) = result.clone() {
