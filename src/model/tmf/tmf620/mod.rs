...
pub async fn tmf620_post_handler(
    ...
    match object.as_str() {
        ...
        "productSpecification" => {
            let mut specification: ProductSpecification =
        ...
        "productOffering" => {
            let mut offering: ProductOffering =
                serde_json::from_str(json.as_str()).expect("Could not parse ProductOffering");
        ...
        "productOfferingPrice" => {
            let mut price: ProductOfferingPrice =
                serde_json::from_str(json.as_str()).expect("Could not parse productOfferingPrice");
            if price.id.is_none() {
                price.generate_id();
            }
            // Set last update for new records
