...
impl TMF637ProductInventoryManagement {
    ...
    pub async fn get_products(&self, query_ops: QueryOptions) -> Result<Vec<Product>, PlatypusError> {
        self.persist.as_ref().unwrap().get_items(query_ops).await
    }

    pub async fn get_product(&self, id: String, query_ops: QueryOptions) -> Result<Vec<Product>, PlatypusError> {
    ...
    pub async fn update_product(&self, id: String, patch: Product) -> Result<Vec<Product>, PlatypusError> {
    ...
    pub async fn delete_product(&self, id: String) -> Result<Product, PlatypusError> {
        let result = self.persist.as_ref().unwrap().delete_tmf_item::<Product>(id).await;
        #[cfg(feature = "events")]
        {
